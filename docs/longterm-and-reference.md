...
下一步：原计划的消融评测实验（多场景 × 多模型）是天然的观测场，但它已于 2026-09-08 取消（`evaluation-layer-pivot.md`）。目前只有 13 条 CLI 运行的 trace 可翻，没有可复现的模式，有证据再议。如果证实，它可能成为维度 A 的第四条方向，也可能被追认为 A1/A3 已经解释的现象的一个侧面——由届时的证据决定。
...
### 14. 「A3 不适合做 P0 精炼」的机制可能说错了（2026-08-30 立）

**〔面试可选〕** 被问"你怎么验证方法论里的论断"时可用——它是第一条靠**抓模型
推理过程**验证出来的观察，只看最终输出发现不了。

第六章现在的表述是：A3（任务优先）的模型"把想法说得更好"会被理解成任务的一部分，
于是**主动推理和扩展**，不适合做忠实精炼。

2026-08-30 工作流引擎的跑批实测（`dev-diagnose` 场景，原 seed 约 100 字）：
...
关键在 glm-5.3。抓到的推理过程显示：它逐条核对了硬性要求、甚至讨论了"忠实度
信号"，**内容一个字没扩展**。它扩展的是**任务边界**——思考的最后阶段自己决定
"Format: refined text + short note on changes"。

**更准确的表述可能是**：不是"A3 会扩展内容"，是"**重推理模型倾向于自己决定
输出格式**"。而 A2 权限优先的特点恰恰是**没授权的事不做**——这反过来给"为什么
P0 可以用 A2"提供了一个 docs 里没有的理由。

**结论没错，机制说错了**。所以这条不构成本体修订，只是措辞的精确化候选。

**证据强度**：单场景、三个模型、一次。远不到"大多数情况下有效"的门槛。

下一步：多场景观察，特别是看 A1（Claude）在同一任务上的表现——如果 A1 也不加
元层说明，那"规范优先"和"权限优先"在忠实精炼上是等价的，第六章的论证需要重写。

---

### 15. 方法论的规定有预测力：idea.md 与 decision-log 分离（2026-08-30 立）

**〔面试主证据〕** 这条能回答"你这套东西是拍脑袋想的还是真有用"——方法论里一条
看起来像洁癖的规定，提前说出了违反之后会发生什么，而实测一字不差地复现了。
...
工作流引擎第一版实现违反了这条——fix 步骤的产出（批判响应日志 + 修订后的方案）
被整个写进同一个 `idea-v2.md`，文件以「# 批判响应日志」开头，日志占全文 21%。

**实测后果与预言完全一致**：P2B 零上下文单盲读完之后，给出的批准理由是

> "文档对 7 个批判点做了具体且逻辑自洽的回应"

——它在评价**修改过程**，不是在评价**产品**。而它的第一条卡点也正是这个：
"文档开头直接是批判响应日志，没有说明这是什么类型的文档、谁提出的批判、
原方案是什么"。

**含义**：这条规定不是形式主义。它防的失败模式是真实存在的，而且会在自动化
实现里悄悄发生——因为让模型"先说怎么改的、再给方案"是最自然的 prompt 写法，
不刻意分离就会污染。

**对方法论的价值**：这是本体规定第一次在"被违反"的情况下被验证。之前的实验都是
遵守规则跑的，验证的是"遵守了有没有用"；这次是无意中违反了，验证的是"不遵守会
怎样"——后者的说服力更强。

已在实现层修复（`---IDEA-BELOW---` 分隔 + 编排器切分），详见 `engine/VALIDATION.md`
检验六。

### 16. 模型会用实质判断覆盖形式规则 · Kill shot 分级的实测失效（2026-08-30 立）

**〔面试主证据〕** 这是"规则化不能替代人做边界判断"的直接实证，而且是**旗舰模型**
跑出来的——堵死了"换个更强的模型就行了"这个反驳。它把变种 5.2 节那条"已知限制"
往前推了一大步。

`liangyi-auto-variant.md` 5.2 节记的已知限制是：

> 分级标准（K/F、H/M）边界有主观空间、不同 Claude-2 实例可能给不同分级。

2026-08-30 工作流引擎跑同一场景，实测到的是**更严重的一种失效**。旗舰配置下
2D-fix 的分级表第 3 行：
...
规则写得毫不含糊：Kill shot = 框架级（K）+ 具体性高（H）。**它自己标了 K 和 H，
然后判成了 Fixable**，理由是"仍属于客户、价值包装和计费方式可验证、可调整的产品
框架内问题"。最终结论"没有达到 kill shot"，产出 v5。

**这不是边界模糊导致标注不同，是标注一致但拒绝按标注执行。** 模型用自己的实质
判断覆盖了形式规则。

**跨模型对照**（同一场景、同一套规则）：

| | smoke 档（glm-4-flash） | primary 档（gpt-5.6-luna） |
|---|---|---|
| 分级 | 5 条全判 K+H | 4 条 F，1 条标 K 却判 Fixable |
| 判定 | **回 P1** | **产出 v5** |

**含义**：Kill shot 分级是变种为了"把边界决策规则化、好让 AI 自动做"而发明的
补丁。这次实测说明——**规则化并没有消除边界判断，只是把它藏进了「模型愿不愿意
遵守规则」里**。全自动模式下没有人来裁决"规则说 A、判断说 B"这个冲突，而模型
会安静地选择自己的判断，还给出看起来很合理的理由。

**与立场三的关系**：这条从一个新角度支持了"边界决策不可委托"。四次变种实验证明
的是"AI 在这个位置会卡死"，这次证明的是"**就算给它明确的规则让它不卡死，它也会
在规则和判断冲突时自行其是**"。前者是能力问题，后者是权限问题——而权限问题不会
因为模型变强而消失。

**一个诚实的补充**：从产品角度看，这次模型的判断未必是错的（那 5 条论据确实可能
框架内消化）。问题不在判得对不对，在于**它绕过了自己被赋予的规则，而没有任何机制
让这件事被发现**——是我事后逐条核对分级表才看出来的。

**证据强度**：单场景、两个模型、一次对照。不到本体修订门槛，作为候选观察记录。

下一步：Phase E 跑五个场景时统计这类"标注与判定不一致"的出现频率。如果稳定复现，
它可能比"P2D-fix 死循环"更能说明边界决策的性质。

### 17. 评测方法本身必须先被评测 · 绝对打分不可用（2026-08-30 立）

**〔面试主证据〕** 这条回答一个很硬的追问——"**你怎么知道你的评测是可信的**"。
答案不是"我用了两个裁判"，而是"我先测了评测方法本身，发现它不可信，然后改掉了"。

**原设计**：消融实验用 5 个维度各 1-10 分打绝对分，两个不同坐标的裁判盲评，
每对跑 3 次看稳定性。

**第一次实测就崩了**。同一组文档（`idea-v1` 对 `idea-v5`），五个裁判给出的
版本差值从 −4 到 +4：
...
有说链条让方案变好的，有说变差的。**结论完全取决于选了谁当裁判**。

**追查根因用的是自一致性测试**——让同一个裁判对同一份文档打 5 次：

| 裁判 | v1 五次原始分 | v5 五次原始分 | 版本差 | 自身极差 |
|---|---|---|---|---|
| gpt-5.6-luna | 38, 40, 40, 42, 42 | 41, 41, 42, 42, 42 | +1.2 | **4** |
| deepseek-v4-flash | 39, 40, 40, 41, 41 | 36, 39, 39, 40, 42 | −1.0 | **6** |

**裁判自身的噪声（4-6 分）远大于要测的信号（1 分）。** 跨裁判的分歧不是它们
意见不同，是大家都在噪声里各自取样。

根因不在模型能力——换裁判、换更贵的模型都救不了。病在**绝对打分这个方法**：
1-10 分没有锚点，"这算 8 分还是 6 分"全凭当次心证。

**改成成对比较后问题消失**（同样的文档、同样的裁判）：

| | 绝对打分 | 成对比较 |
|---|---|---|
| 两个裁判的结论 | +1.2 与 −1.0，互相矛盾 | **6:0 一致判 v5 更好** |
| 12 次比较的分歧 | — | **零分歧**（除一个维度） |
| 位置偏差 | — | 两裁判均 3/6 选 A 位，无偏好 |

（正反顺序各跑一半来抵消位置偏差——有些模型系统性偏好选 A。）

**唯一的分歧维度反而最有信息量**：目标用户清晰度上 deepseek 判 v1 更好（4:2）、
luna 判不出。其余四维 v5 全胜。这与 P2C 当时的诊断吻合——它指出目标用户从
"没有专职运维的小团队"收窄为"已有错误采集能力的小团队"，而 2C-rollback 只做了
部分回退。**所以这不是噪声，是链条的真实副作用被测出来了**：方案在完整性、
可行性、风险覆盖、自洽性上都改善了，代价是目标用户变模糊。

**对方法论的含义**：方法论一直强调"用 divergence 防自评偏差"，实践中还要加一条
——**divergence 只在测量本身可靠时才有意义**。两个坐标不同的裁判如果都在噪声里
取样，分歧就不再是信息，只是随机。测量工具要先被验证，然后才谈用它测什么。

**证据强度**：单场景、五个裁判、两轮方法对照。方法层面的结论（绝对打分不如
成对比较）在评测领域有广泛先例，但在这套方法论的语境下是首次实测。

### 17. 决策点的判断层次错了，会把有意图的人也变成橡皮图章（2026-08-31 立）

**〔面试主证据〕** 这条回答"你怎么保证人不是在走过场"——而且答案不是"我们相信人会
认真"，是"我们改了问法，让他答得了"。

工作流的第一版决策界面把整篇 P2C 漂移诊断（3000+ 字）摊给使用者。Wenbo 的反馈：

> 我没有这个能力和资格去判断……这几个实验里面包含的领域都不是我擅长的领域。

**诊断**：界面在要求他做**领域专家的设计评审**（"权限模型该怎么拆""自动回滚
放哪一档"），但方法论要他做的是**意图所有者的守护判断**。docs 对 2C-rollback
的定义是「回退的标准是**这偏离了产品最初要解决的问题**」——即"你当初说要 X，
现在变成了 Y，认不认"。

改成这个问法之后，同一个人、同一个场景：

> 我其实就可以去判断……它不算是那种特别技术性或者是具有太高判断要求的问题。

**这条为什么重要**：它揭示了「反滑坡规则」此前没覆盖的一种滑坡形态。原规则防的是
"人累了 → 说你看着办"，是**注意力**耗尽；这里是"人不懂 → 只能点接受"，是**判断
依据**缺失。两者的表象一模一样——决策日志上都写着"人已确认"，**从记录上完全区分
不出来**。

**与立场三的关系**：立场三精确化后说"人做的是系统内部无法自判的决策"。这条给它
补了一个操作推论——**界面必须把判断收敛到那个层面，否则立场三在实现层会自动失效**。
系统知道"哪个设计更严谨"（可自判），系统不知道"你想要什么"（不可自判），
问错了人就只能装作在决策。

**证据强度**：单场景、一次改造前后对照。但对照本身很干净——同一个人、同一份材料、
只改了问法，判断能力从"没有资格"变成"可以判断"。

---

### 18. 归档产物看不出人介入过（2026-08-31 立）

**〔面试主证据〕** 这条是"AI 溶解人的判断"最难防的性质的直接证据，而且是找素材时
意外撞见的，不是推理出来的。

为了验证范围扩大化检测器，我去翻决策模拟器 round-2 的记录——那次 critic 只说
"双语同口径在心理类无成功先例"，Claude-2 却把整个英文版砍了，Wenbo 当场拦下。

**结果发现：归档的 `r2-idea-v2.md` 里英文版好好地在，还写得更详细了。**

因为归档的是**人修正之后**的版本。AI 那个错误反应只存在于
`我和claude-2的完整对话-round2.md` 里——如果当时没人拦，"砍掉英文版"会直接固化
进 v2，然后一路传到 v5，**而事后从任何产物中都看不出这件事发生过**。

**含义**：AI 溶解人判断的可怕之处不在于它会犯错，在于**它的错误不留痕迹**。
一份被悄悄砍掉一半市场的方案，读起来和一份本来就只做单一市场的方案完全一样，
自洽、通顺、有理有据。事后复盘无从下手，因为没有"复盘对象"。

**这也是范围扩大化检测器存在的理由**：它不是为了替人做判断，是为了**让这件事
留下痕迹**——触发时会写进 decision-log，哪怕人最后选了"接受"，记录里也有
"这里 AI 想删掉 X，批判其实只说了 Y"。

**对产品形态的含义**：trace 可视化里那个"AI 想干什么被拦住"的节点，展示的正是
这个原本不可见的东西。这比任何数据都更能说明人在场的价值。
